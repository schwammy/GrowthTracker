...
{
    List<AddCompetencyDto> ReadAllCompetencies();
    List<AddCompetencyDto> ReadAllRoleCompetencies(string role);
}

public class ExcelReader : IExcelReader
{
    public List<AddCompetencyDto> ReadAllCompetencies()
    {
    ...
    public List<AddCompetencyDto> ReadAllRoleCompetencies(string role)
    {
        List<AddCompetencyDto> list = new List<AddCompetencyDto>();
        using (SpreadsheetDocument? spreadSheetDocument = SpreadsheetDocument.Open(@"C:\Users\andys\OneDrive\Documents\Normalized Developer Matrix.xlsx", false))
        {
            ...
            foreach (Row row in rows.Skip(1))
            {
                var roles = GetCellValue(spreadSheetDocument, row.Descendants<Cell>().ElementAt(9));
                var listOfRoles = roles.Split(",");
                if (!listOfRoles.Any(r => r.Trim() == role))
                {
                    continue;
                }
